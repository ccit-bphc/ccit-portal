...
def previous(request):
    ...
    if user.is_staff or user.is_nucleus:
        complaints=Complaint.objects.filter(status=Complaint.REGISTERED).filter(status=Complaint.TAKEN_UP,handler=user)filte
...
def register_complaint(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(request,"Please fill all the details correctly in the form provided")
            return render(request, "complaints/complaints_register.html")
    else:
        form = ComplaintForm()
        return render(request, "complaints/complaints_register.html")
